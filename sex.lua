...
--// psa: this is skidded off niggerbeater and LUAU and INCOGNITO AND GLIX AND STA (MULTI SKIDDED)
--// this was NOT made by me XDDDDDDDDD
--// maybe thats why it looks so shit
...
function sandbox:initialize()
	...
	environment.global.getscripts = function()
		...
		environment.global.fireproximityprompt = function(Obj, Amount, Skip)
			Amount = Amount or 1
...
local function parse_file_path(dir_path: string, no_sanity: boolean): { string }
	...
	environment.global.readfile = function(dir_path: string): string?
...
environment.global.writefile = function(dir_path: string, content: string)
    assert(type(dir_path) == "string", "arg #1 must be type string")
...
environment.global.appendfile = function(dir_path: string, content: string)
    assert(type(dir_path) == "string", "arg #1 must be type string")
    assert(type(content) == "string", "arg #2 must be type string")
...
environment.global.isfile = function(dir_path: string)
    assert(type(dir_path) == "string", "arg #1 must be type string")
...
environment.global.isfolder = function(dir_path: string)
    assert(type(dir_path) == "string", "arg #1 must be type string")
...
environment.global.listfiles = function(dir_path: string)
    assert(type(dir_path) == "string", "arg #1 must be type string")
	...
	for env_name, env in environment do
		cloned_environment[env_name] = table.clone(env)
	end

	self.environment = cloned_environment
...
function gui:Create()
	local ASEUI = Instance.new("ScreenGui") -- gotta use a lua gui (websockets are fucked)
	-- if ur seeing this just fucking remove it its useless
...
ASEUI.Name = "ASEUI"
ASEUI.Parent = game.CoreGui
ASEUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
ASEUI.DisplayOrder = 10
ASEUI.ResetOnSpawn = false

Frame.Parent = ASEUI
Frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
Frame.BorderSizePixel = 0
Frame.Position = UDim2.new(0.278955996, 0, 0.293316811, 0)
Frame.Size = UDim2.new(0, 542, 0, 333)

UIAspectRatioConstraint.Parent = Frame
UIAspectRatioConstraint.AspectRatio = 1.628

Frame_2.Parent = Frame
Frame_2.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
Frame_2.BorderColor3 = Color3.fromRGB(0, 0, 0)
Frame_2.BorderSizePixel = 0
Frame_2.Position = UDim2.new(0.0202952009, 0, 0.111111097, 0)
Frame_2.Size = UDim2.new(0.857933581, 0, 0.804804802, 0)
...
TextBox.PlaceholderText = "Execute here"
TextBox.Text = ""
...
ButtonList.Position = UDim2.new(0.02, 0, 0.9, 0)
ButtonList.Size = UDim2.new(0.96, 0, 0.1, 0)

UIGridLayout.Parent = ButtonList
UIGridLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
UIGridLayout.SortOrder = Enum.SortOrder.LayoutOrder
UIGridLayout.CellPadding = UDim2.new(0.1, 0, 0, 0)
UIGridLayout.CellSize = UDim2.new(0.45, 0, 1, 0)
...
TextLabel_2.Size = UDim2.new(1, 0, 0.111111097, 0)
TextLabel_2.Font = Enum.Font.GothamBold
TextLabel_2.Text = "ASE"
...
local function executeScript() -- execute.LocalScript 
	local script = Instance.new('LocalScript', execute)

	script.Parent.MouseButton1Down:Connect(function()
		sandbox.environment.global.loadstring(script.Parent.Parent.Parent.Frame.ScrollingFrame.TextBox.Text)()
	end)
end
coroutine.wrap(executeScript)()

local function clearScript() -- clear.LocalScript 
	local script = Instance.new('LocalScript', clear)

	script.Parent.MouseButton1Down:Connect(function()
		script.Parent.Parent.Parent.Frame.ScrollingFrame.TextBox.Text = ""
	end)
end
coroutine.wrap(clearScript)()

local function toggleUI() -- ASEUI.LocalScript 
	local script = Instance.new('LocalScript', ASEUI)
...
local function dragUI() -- Frame.DragScript
